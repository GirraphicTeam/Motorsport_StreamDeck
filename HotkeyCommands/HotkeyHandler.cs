...
namespace HotkeyCommands
{
    public static class HotkeyHandler
    {
        ...
        public static void RunHotkey(string hotkey)
        {
            ...
            {
                SimulateKeyStroke(keyStrokes.Select(ks => ks.KeyCode).ToArray());
            }
        }

        private static void SimulateKeyStroke(VirtualKeyCode[] keyStrokes)
        {
            WindowsInputLib.InputSimulator iis = new WindowsInputLib.InputSimulator();
            iis.Keyboard.KeyDown(keyStrokes); Thread.Sleep(10);
            iis.Keyboard.KeyUp(keyStrokes);
        }
